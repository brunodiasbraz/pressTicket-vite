...
export const restartPm2 = async (
  ...
  if (import.meta.env.PM2_FRONTEND && import.meta.env.PM2_BACKEND) {
    setTimeout(function () {
      const { execSync } = require("child_process");
      execSync(`pm2 restart ${import.meta.env.PM2_FRONTEND}`);
      execSync(`pm2 restart ${import.meta.env.PM2_BACKEND}`);
    }, 100);
